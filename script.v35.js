...
/* ===================================================================
   ...
   =================================================================== */
/* sync */
function initSync(){var b=document.getElementById("syncSettingsBtn");if(!b)return;b.style.display="inline-flex";b.textContent="☁️";if(CloudSync.getToken()){b.textContent="⏳";b.title="☁️ 同步中...";CloudSync.pull().then(function(c){if(c&&c.pets){var l=PetStore.getAll();var m={};l.forEach(function(p){m[p.id]=p});c.pets.forEach(function(p){if(!m[p.id])l.push(p)});PetStore.save(l);refreshPets();if(c.records)localStorage.setItem("pet_receipt_records",JSON.stringify(c.records));if(document.getElementById("petGrid"))initIndexPage();if(document.getElementById("petDetail"))initDetailPage();b.textContent="✅";b.title="☁️ 已同步";setTimeout(function(){b.textContent="☁️"},2000)}else{b.title="☁️ 云端暂无数据"}}).catch(function(){b.title="☁️ 网络不通"});window._triggerSync=function(){CloudSync.push(CloudSync.pack()).catch(function(){})}}else{b.title="☁️ 点击设置同步"}b.addEventListener("click",function(){if(CloudSync.getToken()){b.textContent="⏳";CloudSync.push(CloudSync.pack()).then(function(o){b.textContent="☁️";if(o)Toast.show("☁️ 已同步","success");else Toast.show("同步失败，开VPN或检查网络","error")})}else{showTokenPrompt()}})}
function showTokenPrompt(){var ov=document.createElement("div");ov.className="confirm-overlay";ov.innerHTML='<div class="confirm-dialog" style="max-width:340px;text-align:left;"><div style="font-size:1rem;margin-bottom:8px;">🔑 输入 GitHub Token</div><p style="font-size:0.7rem;color:var(--ink-secondary);margin-bottom:10px;">Token 只存浏览器，不上传源码。<br><a href="https://github.com/settings/tokens" target="_blank">获取 Token →</a></p><input class="form-input" id="tkInput" placeholder="ghp_..." style="margin-bottom:10px;font-size:0.8rem;"><div style="display:flex;gap:8px;"><button class="confirm-cancel" id="tkCancel" style="flex:1;">取消</button><button class="confirm-delete" id="tkSave" style="flex:1;background:var(--tag-comm-text);">保存</button></div></div>';document.body.appendChild(ov);ov.addEventListener("click",function(e){if(e.target===ov)ov.remove()});ov.classList.add("open");ov.querySelector("#tkCancel").onclick=function(){ov.remove()};ov.querySelector("#tkSave").onclick=function(){var t=ov.querySelector("#tkInput").value.trim();if(!t)return;localStorage.setItem("gh_sync_token",t);ov.remove();Toast.show("✅ Token 已保存！刷新后生效","success");window._triggerSync=function(){CloudSync.push(CloudSync.pack()).catch(function(){})}}}// end sync
