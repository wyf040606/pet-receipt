...
/* ===================================================================
   CloudSync — GitHub API 跨设备同步
   =================================================================== */
const CloudSync = {
  RAW: 'https://raw.githubusercontent.com/wyf040606/pet-receipt/main/pets-data.json',
  API: 'https://api.github.com/repos/wyf040606/pet-receipt/contents/pets-data.json',

  getToken() { return localStorage.getItem('gh_sync_token'); },

  /** 拉取云端数据 */
  async pull() {
    try {
      const r = await fetch(this.RAW + '?t=' + Date.now(), { cache: 'no-store' });
      if (!r.ok) return null;
      return await r.json();
    } catch(e) { return null; }
  },
  ...
  async push(data) {
    ...
    try {
      ...
      let sha = null;
      try {
        const r = await fetch(this.API, { headers: { Authorization: 'token ' + token } });
        if (r.ok) { const i = await r.json(); sha = i.sha; }
      } catch(e) {}
      // 写入
      const content = btoa(unescape(encodeURIComponent(JSON.stringify(data, null, 2))));
      const body = { message: '☁️ sync', content };
      if (sha) body.sha = sha;
